...
import { useAppState, FOCUS_DURATIONS } from '../../state/AppState';
...
export default function FocusSetupScreen() {
  const { state, cycleFocusDuration, toggleFocusApp } = useAppState();
  const insets = useSafeAreaInsets();
  const selectedCount = RESTRICT_APPS.filter((a) => state.focusApps[a]).length;
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingBottom: insets.bottom + 34 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginTop: 18, borderWidth: 1, borderColor: colors.cardBorder, borderRadius: 24, backgroundColor: '#FFFFFF', overflow: 'hidden' }}>
          <PressableScale
            ...
            accessibilityLabel="Worship goal, Durood Sharif 100. Double tap to change."
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Text style={{ fontSize: 14, color: colors.inkMuted }}>Durood Sharif · {state.newTarget}</Text>
              <ChevronRightIcon />
            </View>
          </PressableScale>
          ...
          <PressableScale
            ...
            scaleTo={1}
            accessibilityRole="button"
            accessibilityLabel={`Focus duration, ${FOCUS_DURATIONS[state.focusDuration]}. Double tap to change.`}
            style={{ padding: 18, borderBottomWidth: 1, borderColor: colors.cardBorder, minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}
          >
            <Text style={{ fontSize: 15.5, fontWeight: '500', color: colors.inkStrong }}>Focus duration</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <Text style={{ fontSize: 14, color: colors.inkMuted }}>{FOCUS_DURATIONS[state.focusDuration]}</Text>
              <ChevronRightIcon />
            </View>
          </PressableScale>
          ...
          <View style={{ padding: 18 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 14 }}>
              {RESTRICT_APPS.map((a) => {
                const on = state.focusApps[a];
                return (
                  <PressableScale
                    key={a}
                    onPress={() => toggleFocusApp(a)}
                    scaleTo={0.94}
