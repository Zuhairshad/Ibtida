...
export default function GoalNewScreen() {
  ...
  return (
    <ScreenFade duration={280} style={{ backgroundColor: colors.bg, paddingTop: insets.top + 12, paddingHorizontal: 24, paddingBottom: insets.bottom + 20 }}>
      ...
      <Text style={{ ...type.h1, color: colors.ink }}>Create goal</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={{ borderWidth: 1, borderColor: colors.cardBorder, borderRadius: radii.card, backgroundColor: colors.card, ...shadow.card }}>
          <View style={{ padding: 18, borderBottomWidth: 1, borderColor: colors.cardBorder }}>
            ...
            <Text style={{ fontSize: 17, fontWeight: '500', color: colors.ink, marginTop: 9 }}>Durood Sharif</Text>
          ...
          <View style={{ padding: 18, borderBottomWidth: 1, borderColor: colors.cardBorder, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <View>
              ...
              <Text style={{ fontSize: 17, fontWeight: '500', color: colors.ink, marginTop: 9 }}>{target}</Text>
            </View>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <PressableScale onPress={targetDown} scaleTo={0.9} accessibilityRole="button" accessibilityLabel="Decrease daily target" style={{ width: 44, height: 44, borderRadius: radii.control, borderWidth: 1, borderColor: colors.cardBorder, backgroundColor: colors.card, alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 19, color: colors.inkMuted }}>−</Text>
              </PressableScale>
              <PressableScale onPress={targetUp} scaleTo={0.9} accessibilityRole="button" accessibilityLabel="Increase daily target" style={{ width: 44, height: 44, borderRadius: radii.control, borderWidth: 1, borderColor: colors.cardBorder, backgroundColor: colors.card, alignItems: 'center', justifyContent: 'center' }}>
                <Text style={{ fontSize: 19, color: colors.inkMuted }}>+</Text>
              </PressableScale>
            </View>
          </View>
              ...
              {FREQS.map((label, i) => {
                ...
                return (
                  <PressableScale
                    ...
                    onPress={() => setFreq(i)}
                    scaleTo={1}
                    style={{ flex: 1, minHeight: 44, padding: 12, borderRadius: radii.button, backgroundColor: on ? colors.primary : colors.card, borderWidth: on ? 0 : 1, borderColor: colors.cardBorder, alignItems: 'center' }}
                  >
          ...
          >
            <View>
              ...
              <Text style={{ fontSize: 17, fontWeight: '500', color: reminderOn ? colors.ink : colors.inkSecondary, marginTop: 9 }}>
